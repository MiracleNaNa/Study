...
Error_Message = ['Failed to establish a new connection', 'ConnectTimeoutError', "ConnectionResetError", "Remote end closed connection without response"]
...
for line in url:
    ...
    try:
        ...
        if Error_Message[0] in f:
            r = requests.get('https://' + b, headers=header2, timeout=5, allow_redirects=True, verify=False, stream=True)
            for d in r.iter_lines():
                e += d.decode('utf-8')
                break
            if r.status_code == 200:
